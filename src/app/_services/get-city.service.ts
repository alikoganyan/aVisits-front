...
export class GetCityService {
    ...
    getStreet(city: string, street: string) {
        return this.jsonp.request('https://maps.googleapis.com/maps/api/geocode/json?address='+ city + ' ' + street +'&callback=JSONP_CALLBACK&key=AIzaSyD4AD-6nrr-uDDkblQh3TeJhbMfg8xWOys')
            .map(
                (response: Response) => {
                    return response.json()
                }
